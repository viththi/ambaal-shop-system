...
# ============================================================
# AMBAAL SHOP MANAGEMENT SYSTEM - REAL-TIME LIVE V3
# Instant tabs + SSE cross-device synchronization + pooled MySQL.
# ============================================================
...
def db_connection():
    """Borrow a pooled MySQL connection that always reads the latest committed data."""
    connection = _get_db_pool().get_connection()
    # Autocommit prevents a pooled read connection from keeping an old REPEATABLE
    # READ snapshot. That stale snapshot was the reason a saved balance could say
    # Rs. 6,000 in the flash message while the card still showed Rs. 5,000.
...
# Server-Sent Events (SSE) lets every open phone/laptop know immediately when
# another device changes application data. The browser then refreshes only the
# page that is currently visible; there is no manual refresh and no 1-second
# database polling loop.
_realtime_subscribers = set()
_realtime_lock = Lock()
_realtime_revision = 0


def _broadcast_realtime_change(kind="data_changed"):
    """Push one small change notification to every connected browser."""
    global _realtime_revision
    with _realtime_lock:
        _realtime_revision += 1
        revision = _realtime_revision
        subscribers = list(_realtime_subscribers)

    # IMPORTANT: SSE requires REAL newline characters, not the two-character\n sequence.
    ...
    if dead:
        with _realtime_lock:
            for subscriber in dead:
                _realtime_subscribers.discard(subscriber)


@app.route("/events")
def realtime_events():
    """Keep a lightweight live connection open for cross-device updates."""
    if "user_id" not in session:
        return Response(status=401)
    subscriber = Queue(maxsize=20)
    with _realtime_lock:
        _realtime_subscribers.add(subscriber)

    def stream():
        try:
            # Tell EventSource to reconnect quickly if a mobile network changes.
            yield f"retry: 1200\nevent: ready\ndata: {_realtime_revision}\n\n"
            while True:
                try:
                    yield subscriber.get(timeout=15)
                except Empty:
                    # Heartbeat prevents proxies/mobile networks from closing SSE.
                    yield ": keep-alive\n\n"
        finally:
            with _realtime_lock:
                _realtime_subscribers.discard(subscriber)
...
@app.route("/api/realtime-version")
def realtime_version():
    """Tiny no-database fallback used when a proxy/browser pauses SSE."""
    if "user_id" not in session:
        return jsonify({"revision": -1}), 401
    with _realtime_lock:
        revision = _realtime_revision
...
def dashboard_state_api():
    """Return only the live dashboard counters; much lighter than a full page render."""
    if "user_id" not in session:
    ...
    try:
                ...
                COALESCE((
                    ...
                    FROM (
                        SELECT GREATEST(SUM(CASE
                            ...
                            ELSE 0 END), 0) AS customer_balance
        ...
        return jsonify({
            ...
            "total_balance": str(row.get("total_balance") or 0),
        })
    except Exception as error:
...
@app.after_request
def notify_other_devices_after_write(response):
    """Broadcast after successful data-changing requests have completed."""
    try:
        if (
            ...
            _broadcast_realtime_change("data_changed")
    except Exception:
        pass
    return response
...
def record_authenticated_activity():
    ...
    if "user_id" not in session:
        return
    if request.endpoint in {"pwa_manifest", "pwa_icon", "service_worker", "live_logs_data", "realtime_events", "realtime_version", "dashboard_state_api"}:
        return
...
<script>
    ...
    // -----------------------------------------------------------------\n    // INSTANT TAB + REAL-TIME DEVICE SYNC ENGINE\n    // 1) Main pages are prepared in browser memory so a tab can appear instantly.\n    // 2) Every GET is revalidated with cache:no-store.\n    // 3) SSE (/events) tells all open phones/laptops immediately after a write.\n    // 4) The currently visible page then reloads quietly from RDS without a\n    //    manual browser refresh or a 1-second polling loop.\n    // -----------------------------------------------------------------\n    const fastPageCache = new Map();\n    const FAST_CACHE_MS = 60000;\n    let fastNavigationController = null;\n    let realtimeSource = null;\n    let realtimeRefreshTimer = null;\n    let realtimeRefreshRunning = false;\n    let realtimeRefreshPending = false;\n    let realtimeRevision = null;\n    let realtimePollTimer = null;\n\n    function fastProgressStart(trigger) {\n        const bar = document.getElementById("fastProgress");\n        if (bar) bar.className = "fast-progress show";\n        if (trigger) {\n            trigger.classList.add("fast-busy");\n            trigger.setAttribute("aria-busy", "true");\n        }\n    }\n\n    function fastProgressDone() {\n        const bar = document.getElementById("fastProgress");\n        document.querySelectorAll(".fast-busy").forEach(el => {\n            el.classList.remove("fast-busy");\n            el.removeAttribute("aria-busy");\n        });\n        if (bar) {\n            bar.className = "fast-progress done";\n            setTimeout(() => { bar.className = "fast-progress"; bar.style.width = ""; }, 140);\n        }\n    }\n\n    function fastCanHandleUrl(rawUrl) {\n        try {\n            const u = new URL(rawUrl, location.href);\n            if (u.origin !== location.origin) return false;\n            if (u.pathname === "/logout" || u.pathname === "/events" || u.pathname.startsWith("/all-data/export-excel")) return false;\n            if (/\\.(?:xlsx?|csv|pdf|zip|png|jpe?g|webp)$/i.test(u.pathname)) return false;\n            return true;\n        } catch (_) { return false; }\n    }\n\n    function cacheKey(rawUrl) {\n        const u = new URL(rawUrl, location.href);\n        u.hash = "";\n        return u.href;\n    }\n\n    function cacheSet(url, html) {\n        fastPageCache.set(cacheKey(url), { html, time: Date.now() });\n        if (fastPageCache.size > 24) fastPageCache.delete(fastPageCache.keys().next().value);\n    }\n\n    function cacheGet(url) {\n        const key = cacheKey(url);\n        const item = fastPageCache.get(key);\n        if (!item) return null;\n        if (Date.now() - item.time > FAST_CACHE_MS) {\n            fastPageCache.delete(key);\n            return null;\n        }\n        return item.html;\n    }\n\n    function fastApplyDocument(html, url, pushHistory=true, preserveScroll=false) {\n        const parsed = new DOMParser().parseFromString(html, "text/html");\n        const incomingMain = parsed.querySelector(".main");\n        const currentMain = document.querySelector(".main");\n        const incomingSidebar = parsed.querySelector(".sidebar");\n        const currentSidebar = document.querySelector(".sidebar");\n        const incomingBottom = parsed.querySelector(".bottom-nav");\n        const currentBottom = document.querySelector(".bottom-nav");\n        if (!incomingMain || !currentMain) { location.href = url; return; }\n        const oldY = window.scrollY;\n        currentMain.innerHTML = incomingMain.innerHTML;\n        if (incomingSidebar && currentSidebar) currentSidebar.innerHTML = incomingSidebar.innerHTML;\n        if (incomingBottom && currentBottom) currentBottom.innerHTML = incomingBottom.innerHTML;\n        document.title = parsed.title || document.title;\n        closeMenu();\n        if (pushHistory) history.pushState({ fast: true }, "", url);\n        if (preserveScroll) window.scrollTo({ top: oldY, behavior: "instant" });\n        else window.scrollTo({ top: 0, behavior: "instant" });\n    }\n\n    async function networkFetchHtml(url, options={}) {\n        const headers = new Headers(options.headers || {});\n        headers.set("X-Fast-Navigation", "1");\n        headers.set("Cache-Control", "no-cache");\n        const response = await fetch(url, {\n            ...options, headers, credentials: "same-origin", cache: "no-store"\n        });\n        const html = await response.text();\n        return { response, html, finalUrl: response.url || url };\n    }\n\n    async function fastFetchPage(url, options={}, trigger=null, pushHistory=true) {\n        const absolute = new URL(url, location.href).href;\n        const method = (options.method || "GET").toUpperCase();\n        const cached = method === "GET" ? cacheGet(absolute) : null;\n\n        // IMPORTANT: paint cached/prefetched HTML before waiting for RDS.\n        // The tab therefore reacts immediately to the user's tap.\n        if (cached) {\n            fastApplyDocument(cached, absolute, pushHistory, false);\n            pushHistory = false;\n        } else {\n            fastProgressStart(trigger);\n        }\n\n        if (fastNavigationController) fastNavigationController.abort();\n        fastNavigationController = new AbortController();\n        try {\n            const result = await networkFetchHtml(absolute, { ...options, signal: fastNavigationController.signal });\n            if (result.response.ok && result.response.headers.get("content-type")?.includes("text/html")) {\n                if (method === "GET") cacheSet(result.finalUrl, result.html);\n                else fastPageCache.clear();\n                fastApplyDocument(result.html, result.finalUrl, pushHistory, Boolean(cached));\n            } else {\n                location.href = result.finalUrl;\n            }\n        } catch (error) {\n            if (error.name !== "AbortError" && !cached) location.href = absolute;\n        } finally {\n            fastNavigationController = null;\n            fastProgressDone();\n        }\n    }\n\n    document.addEventListener("click", (event) => {\n        const link = event.target.closest("a[href]");\n        if (!link) return;\n        if (event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;\n        if (link.target === "_blank" || link.hasAttribute("download")) return;\n        if (!fastCanHandleUrl(link.href)) return;\n        event.preventDefault();\n        fastFetchPage(link.href, { method: "GET" }, link, true);\n    });\n\n    document.addEventListener("submit", (event) => {\n        const form = event.target;\n        if (!(form instanceof HTMLFormElement)) return;\n        if (form.target === "_blank" || form.dataset.noFast === "1") return;\n        const action = form.action || location.href;\n        if (!fastCanHandleUrl(action)) return;\n        if (!form.reportValidity()) { event.preventDefault(); return; }\n        event.preventDefault();\n        const method = (form.method || "GET").toUpperCase();\n        const submitter = event.submitter || form.querySelector('[type="submit"]');\n        if (method === "GET") {\n            const u = new URL(action, location.href);\n            new FormData(form).forEach((value, key) => { if (String(value).length) u.searchParams.set(key, value); });\n            fastFetchPage(u.href, { method: "GET" }, submitter, true);\n        } else {\n            // Do not let any old balance survive a successful write.\n            fastPageCache.clear();\n            fastFetchPage(action, { method, body: new FormData(form) }, submitter, true);\n        }\n    });\n\n    window.addEventListener("popstate", () => {\n        if (fastCanHandleUrl(location.href)) fastFetchPage(location.href, { method: "GET" }, null, false);\n    });\n\n    function mainNavUrls() {\n        const links = [...document.querySelectorAll('.bottom-nav a[href], .sidebar a.nav-link[href]')];\n        return [...new Set(links.map(a => a.href).filter(fastCanHandleUrl))];\n    }\n\n    // Prepare all main screens early. Requests start close together, so after\n    // login the bottom tabs become memory-fast instead of waiting on a round trip.\n    function prefetchMainPages(force=false) {\n        mainNavUrls().forEach((url, index) => {\n            if (!force && cacheGet(url)) return;\n            setTimeout(() => {\n                networkFetchHtml(url, { headers: { "X-App-Prefetch": "1" } })\n                    .then(r => {\n                        if (r.response.ok && r.response.headers.get("content-type")?.includes("text/html")) {\n                            cacheSet(r.finalUrl, r.html);\n                        }\n                    })\n                    .catch(() => {});\n            }, 250 + index * 450);\n        });\n    }\n\n    function userIsEditing() {\n        const active = document.activeElement;\n        return Boolean(active && /^(INPUT|SELECT|TEXTAREA)$/.test(active.tagName));\n    }\n\n    async function realtimeRefreshCurrentPage() {\n        if (realtimeRefreshRunning) { realtimeRefreshPending = true; return; }\n        if (document.hidden || !navigator.onLine) return;\n        if (userIsEditing()) {\n            realtimeRefreshPending = true;\n            return;\n        }\n        const path = location.pathname;\n        const livePath = path === "/dashboard" || path === "/customer-loans" ||\n            path.startsWith("/customer-loans/customer/") || path === "/shop-items" ||\n            path === "/prices" || path === "/all-data" || path === "/live-logs";\n        if (!livePath) return;\n\n        realtimeRefreshRunning = true;\n        try {\n            const visibleUrl = location.href;\n            const result = await networkFetchHtml(visibleUrl, { headers: { "X-Realtime-Refresh": "1" } });\n            if (result.response.ok && result.response.headers.get("content-type")?.includes("text/html")) {\n                cacheSet(result.finalUrl, result.html);\n                if (location.href === visibleUrl || location.href === result.finalUrl) {\n                    fastApplyDocument(result.html, result.finalUrl, false, true);\n                }\n            }\n        } catch (_) {\n        } finally {\n            realtimeRefreshRunning = false;\n            if (realtimeRefreshPending) {\n                realtimeRefreshPending = false;\n                clearTimeout(realtimeRefreshTimer);\n                realtimeRefreshTimer = setTimeout(realtimeRefreshCurrentPage, 120);\n            }\n        }\n    }\n\n    function formatMoneyLive(value) {\n        const n = Number(value || 0);\n        return "Rs. " + n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });\n    }\n\n    async function refreshDashboardCountersOnly() {\n        if (location.pathname !== "/dashboard" || document.hidden) return false;\n        try {\n            const response = await fetch("{{ url_for('dashboard_state_api') }}", {\n                credentials: "same-origin", cache: "no-store", headers: { "Cache-Control": "no-cache" }\n            });\n            if (!response.ok) return false;\n            const data = await response.json();\n            const customers = document.getElementById("liveCustomerCount");\n            const total = document.getElementById("liveTotalBalance");\n            const items = document.getElementById("liveItemCount");\n            if (customers) customers.textContent = data.customer_count;\n            if (total) total.textContent = formatMoneyLive(data.total_balance);\n            if (items) items.textContent = data.item_count;\n            return true;\n        } catch (_) {\n            return false;\n        }\n    }\n\n    function handleRealtimeChange() {\n        // Keep already-prefetched tabs so taps remain instant. The visible page\n        // is refreshed live; when another tab is opened, its cached version paints\n        // immediately and the normal navigation request revalidates it.\n        clearTimeout(realtimeRefreshTimer);\n        if (location.pathname === "/dashboard") {\n            refreshDashboardCountersOnly().then(ok => {\n                if (!ok) realtimeRefreshCurrentPage();\n            });\n        } else {\n            realtimeRefreshTimer = setTimeout(realtimeRefreshCurrentPage, 30);\n        }\n    }\n\n    function startRevisionFallback() {\n        if (realtimePollTimer) return;\n        const poll = async () => {\n            try {\n                if (!document.hidden && navigator.onLine) {\n                    const response = await fetch("{{ url_for('realtime_version') }}", {\n                        credentials: "same-origin", cache: "no-store"\n                    });\n                    if (response.ok) {\n                        const data = await response.json();\n                        const revision = Number(data.revision);\n                        if (Number.isFinite(revision)) {\n                            if (realtimeRevision === null) realtimeRevision = revision;\n                            else if (revision !== realtimeRevision) {\n                                realtimeRevision = revision;\n                                handleRealtimeChange();\n                            }\n                        }\n                    }\n                }\n            } catch (_) {}\n            realtimePollTimer = setTimeout(poll, 1000);\n        };\n        realtimePollTimer = setTimeout(poll, 700);\n    }\n\n    // Live cross-device channel. A payment/loan saved on a phone causes an open\n    // laptop Dashboard to refresh automatically as soon as the server commits it.\n    function startRealtimeSync() {\n        if (!("EventSource" in window) || realtimeSource) return;\n        realtimeSource = new EventSource("{{ url_for('realtime_events') }}");\n        realtimeSource.addEventListener("ready", (event) => {\n            const n = Number(event.data);\n            if (Number.isFinite(n)) realtimeRevision = n;\n        });\n        realtimeSource.addEventListener("data_changed", (event) => {\n            const n = Number(event.data);\n            if (Number.isFinite(n)) realtimeRevision = n;\n            handleRealtimeChange();\n        });\n        realtimeSource.onerror = () => {\n            // EventSource reconnects automatically using the retry value sent by Flask.\n        };\n    }\n\n    document.addEventListener("focusout", () => {\n        if (realtimeRefreshPending && !userIsEditing()) {\n            realtimeRefreshPending = false;\n            clearTimeout(realtimeRefreshTimer);\n            realtimeRefreshTimer = setTimeout(realtimeRefreshCurrentPage, 80);\n        }\n    });\n\n    document.addEventListener("visibilitychange", () => {\n        if (!document.hidden) realtimeRefreshCurrentPage();\n    });\n    window.addEventListener("focus", realtimeRefreshCurrentPage);\n\n    if (document.querySelector(".main")) {\n        setTimeout(() => prefetchMainPages(false), 80);\n        startRealtimeSync();\n        startRevisionFallback();\n    }\n\n    // Network status feedback. Database writes still require the Flask server.
...
def dashboard():
    ...
    <div class="grid">
        <div class="card">
            ...
            <div class="stat-value" id="liveCustomerCount">{{ customer_count }}</div>
        </div>
        <div class="card">
            <div class="stat-title">Total Outstanding Loan</div>
            <div class="stat-value" id="liveTotalBalance">Rs. {{ total_balance|money }}</div>
        </div>
        <div class="card">
            <div class="stat-title">Shop Items</div>
            <div class="stat-value" id="liveItemCount">{{ item_count }}</div>
        </div>
    </div>
